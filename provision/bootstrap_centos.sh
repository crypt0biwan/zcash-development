...
yum -y install autoconf wget automake git gcc-c++ libdb4-cxx libdb4-cxx-devel boost-devel openssl-devel
...
if [ ! -f "${ZKSNARK_DIR}/sprout-proving.key" ]; then
	echo "proving key doesn not exist, download it!"
...
if [ ! -f "${ZKSNARK_DIR}/sprout-verifying.key" ]; then
	echo "verify key doesn not exist, download it!"
    curl -o /root/.zcash-params/sprout-verifying.key https://z.cash/downloads/sprout-verifying.key
fi

# copy keys to the right place
cp ${ZKSNARK_DIR}/sprout-proving.key /root/.zcash-params/sprout-proving.key
cp ${ZKSNARK_DIR}/sprout-verifying.key /root/.zcash-params/sprout-verifying.key
...
# sha256 hack
sed -i -- 's/shasum -a 256 --check/sha256sum -c/g' ./zcutil/fetch-params.sh
